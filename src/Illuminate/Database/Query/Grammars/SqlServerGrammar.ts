...
export class SqlServerGrammar extends Grammar {
  ...
  protected compileAnsiOffset (query: Builder, components: Record<string, string>): string {
    ...
    if (this.queryOrderContainsSubquery(query)) {
      query.bindings = this.sortBindingsForSubqueryOrderBy(query) as unknown as Bindings
    }
  ...
  protected queryOrderContainsSubquery (query: Builder): boolean {
    ...
    return Arr.first(query.orders, (value: Record<string, string>) => {
  ...
  protected sortBindingsForSubqueryOrderBy (query: Builder): string[] {
    return Arr.sort(query.bindings, (bindings: any, key: string) => {
  ...
  public wrapTable (table: Expression | string): string {
    ...
    return this.getValue(table as any)
  }
