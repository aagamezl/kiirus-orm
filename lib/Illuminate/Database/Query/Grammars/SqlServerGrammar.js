...
class SqlServerGrammar extends Grammar_1.Grammar {
    ...
    compileColumns(query, columns) {
        ...
        let select = (0, Support_1.isTruthy)(query.distinctProperty) ? 'select distinct ' : 'select ';
        ...
        if ((0, utils_1.isNumeric)(query.limitProperty) && limit > 0 && offset <= 0) {
            select += `top ${limit} `;
        }
     ...
     */
    compileExists(query) {
        const existsQuery = (0, utils_1.clone)(query);
        existsQuery.columns = [];
    ...
    compileFrom(query, table) {
        ...
        if ((0, utils_1.isString)(query.lockProperty)) {
     ...
     */
    compileSelect(query) {
        if ((0, Support_1.isFalsy)(query.offsetProperty)) {
    ...
    queryOrderContainsSubquery(query) {
        ...
        return Arr_1.Arr.first(query.orders, (value) => {
    ...
    sortBindingsForSubqueryOrderBy(query) {
        return Arr_1.Arr.sort(query.bindings, (bindings, key) => {
