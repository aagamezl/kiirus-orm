...
class Grammar extends Grammar_1.Grammar {
    ...
    compileAggregate(query, aggregate) {
        ...
        else if ((0, Support_1.isTruthy)(query.distinctProperty) && column !== '*') {
    ...
    compileHavingBetween(having) {
        ...
        const column = this.wrap(having.column);
        const min = this.parameter((0, helpers_1.head)(having.values));
        const max = this.parameter((0, helpers_1.last)(having.values));
   ...
   */
    compileHavings(query) {
        return 'having ' + this.removeLeadingBoolean((0, helpers_1.collect)(query.havings).map((having) => {
    ...
    compileInsert(query, values) {
        ...
        if (!Array.isArray(values) && !Array.isArray((0, helpers_1.reset)(values))) {
            values = [values];
        }
        ...
        const parameters = (0, helpers_1.collect)(values).map((record) => {
    ...
    compileJoins(query, joins) {
        return (0, helpers_1.collect)(joins).map((join) => {
    ...
    compileWheresToArray(query) {
        return (0, helpers_1.collect)(query.wheres).map((where) => {
    ...
    concatenateWhereClauses(query, sql) {
        const conjunction = query instanceof internal_1.JoinClause ? 'on' : 'where';
    ...
    whereBetween(query, where) {
        const between = (0, Support_1.isTruthy)(where.not) ? 'not between' : 'between';
        const min = this.parameter((0, helpers_1.reset)(where.values));
        const max = this.parameter((0, helpers_1.end)(where.values));
    ...
    whereBetweenColumns(query, where) {
        const between = (0, Support_1.isTruthy)(where.not) ? 'not between' : 'between';
        const min = this.wrap((0, helpers_1.reset)(where.values));
        const max = this.wrap((0, helpers_1.end)(where.values));
        return this.wrap(where.column) + ' ' + between + ' ' + min + ' and ' + max;
    }
    /**
    ...
    whereNested(query, where) {
        ...
        const offset = query instanceof internal_1.JoinClause ? 3 : 6;
