...
const BuildsQueries_1 = require("../Concerns/BuildsQueries");
...
class Builder {
    ...
    constructor(connection, grammar, processor) {
        ...
        (0, use_1.use)(Builder, [Macroable_1.Macroable, BuildsQueries_1.BuildsQueries]);
    ...
    addArrayOfWheres(column, boolean, method = 'where') {
        return this.whereNested((query) => {
            for (const [key, value] of Object.entries(column)) {
                if ((0, utils_1.isInteger)(parseInt(key, 10)) && Array.isArray(value)) {
                    query[method](...value);
                }
                else {
    ...
    async aggregate(functionName, columns = ['*']) {
        ...
        const bindings = (0, utils_1.clone)(this.bindings);
    ...
    clone() {
        ...
        for (const propertyName of Object.getOwnPropertyNames(cloned)) {
            ...
            if (Array.isArray(property) || property instanceof Map) {
                Reflect.set(cloned, propertyName, (0, utils_1.clone)(property));
            }
        }
        return cloned;
    }
    ...
    cloneWithout(properties) {
        return (0, Support_1.tap)(this.clone(), (clone) => {
     ...
     */
    async count(columns = '*') {
        return await this.aggregate('count', Arr_1.Arr.wrap(columns));
    }
    /**
     ...
     */
    flattenValue(value) {
        return Array.isArray(value) ? (0, helpers_1.head)(Arr_1.Arr.flatten(value)) : value;
    }
    /**
    ...
    async getCountForPagination(columns = ['*']) {
        ...
        if ((0, Support_1.isFalsy)(results[0])) {
            return 0;
        }
        else if ((0, utils_1.isPlainObject)(results[0])) {
            return Number(results[0]?.aggregate);
        }
        return Number((0, Support_1.changeKeyCase)(results[0]).aggregate);
    }
    /**
    ...
    async insert(values) {
        ...
        if (!Array.isArray((0, helpers_1.reset)(values)) && !(0, utils_1.isPlainObject)((0, helpers_1.reset)(values)) && !(0, utils_1.isPlainObject)(values)) {
            values = [values];
        }
        this.applyBeforeQueryCallbacks();
        ...
        return await this.connection.insert(this.grammar.compileInsert(this, values), this.cleanBindings(Arr_1.Arr.flatten(values, 1)));
    }
    /**
    ...
    async insertGetId(values, sequence) {
        this.applyBeforeQueryCallbacks();
        if (!Array.isArray((0, helpers_1.reset)(values)) && !(0, utils_1.isPlainObject)(values)) {
            values = [values];
        }
    ...
    async insertOrIgnore(values) {
        ...
        this.applyBeforeQueryCallbacks();
        return await this.connection.affectingStatement(this.grammar.compileInsertOrIgnore(this, values), this.cleanBindings(Arr_1.Arr.flatten(values, 1)));
    }
    /**
    ...
    invalidOperatorAndValue(operator, value) {
        return (0, utils_1.isNil)(value) && this.operators.includes(operator) &&
            !['=', '<>', '!='].includes(operator);
    }
    /**
    ...
    async pluck(column, key) {
        ...
        const queryResult = await this.onceWithColumns((0, Support_1.isFalsy)(key) ? [column] : [column, key], () => {
            return this.processor.processSelect(this, this.runSelect());
        });
        if (queryResult.length === 0) {
            return (0, helpers_1.collect)();
        }
    ...
    pluckFromObjectColumn(queryResult, column, key) {
        ...
        return (0, helpers_1.collect)(results);
    }
    /**
    ...
    reorder(column = '', direction = 'asc') {
        ...
        if ((0, Support_1.isTruthy)(column)) {
   ...
   */
    ...
    stripTableForPluck(column) {
        if ((0, Support_1.isFalsy)(column)) {
            return column;
        }
        const separator = String(column).toLowerCase().includes(' as ') ? ' as ' : '\\.';
        return (0, helpers_1.last)(String(column).split('~' + separator + '~i'));
    }
    /**
    ...
    async value(column) {
        ...
        return result.length > 0 || Object.keys(result).length > 0 ? (0, helpers_1.reset)(result) : undefined;
    }
    /**
    ...
    where(column, operator, value, boolean = 'and') {
        ...
        if (Array.isArray(column) || (0, utils_1.isPlainObject)(column)) {
        ...
        if (column instanceof Function && (0, Support_1.isFalsy)(operator)) {
            return this.whereNested(column, boolean);
        }
        // If the column is a Closure instance and there is an operator value, we will
        // assume the developer wants to run a subquery and then compare the result
        // of that subquery with the given value that was provided to the method.
        if (this.isQueryable(column) && (0, Support_1.isTruthy)(operator)) {
        ...
        if (String(column).includes('->') && (0, Support_1.isBoolean)(value)) {
            value = new Expression_1.Expression((0, Support_1.isTruthy)(value) ? 'true' : 'false');
            if ((0, utils_1.isString)(column)) {
                type = 'JsonBoolean';
            }
        }
    ...
    whereDate(column, operator, value, boolean = 'and') {
        ...
        if (value instanceof Date) {
            value = (0, utils_1.dateFormat)(value, 'YYYY-MM-dd');
        }
    ...
    whereDay(column, operator, value, boolean = 'and') {
        ...
        value = this.flattenValue(value);
        if (value instanceof Date) {
            value = (0, utils_1.dateFormat)(value, 'dd');
        }
    ...
    whereFulltext(columns, value, options = {}, boolean = 'and') {
        const type = 'Fulltext';
        columns = (0, Support_1.castArray)(columns);
    ...
    whereMonth(column, operator, value, boolean = 'and') {
        ...
        value = this.flattenValue(value);
        if (value instanceof Date) {
            value = (0, utils_1.dateFormat)(value, 'MM');
        }
        if (!(value instanceof Expression_1.Expression)) {
    ...
    whereTime(column, operator, value, boolean = 'and') {
        ...
        value = this.flattenValue(value);
        if (value instanceof Date) {
            value = (0, utils_1.dateFormat)(value, 'HH:mm:ss');
        }
    ...
    whereYear(column, operator, value, boolean = 'and') {
        ...
        value = this.flattenValue(value);
        if (value instanceof Date) {
            value = (0, utils_1.dateFormat)(value, 'YYYY');
        }
    ...
    withoutSelectAliases(columns) {
        return columns.map((column) => {
            ...
            return (0, utils_1.isString)(column) && aliasPosition !== -1
