...
const JoinClause_1 = require("../JoinClause");
const __1 = require("./../../");
class Grammar extends __1.Grammar {
    constructor() {
        super(...arguments);
        /**
    ...
    compileComponents(query) {
        ...
        for (const component of this.selectComponents) {
            ...
            if (this.isExecutable(query, property)) {
                const method = 'compile' + lodash_1.capitalize(name);
    ...
    compileHavingBetween(having) {
        ...
        const column = this.wrap(having['column']);
        const min = this.parameter(Helpers_1.head(having.values));
        const max = this.parameter(Helpers_1.last(having.values));
    ...
    compileInsert(query, values) {
        ...
        const parameters = Helpers_1.collect(values).map((record) => {
    ...
    compileJoins(query, joins) {
        return Helpers_1.collect(joins).map((join) => {
    ...
    compileOrdersToArray(query, orders) {
        return orders.map((order) => {
            var _a;
            return (_a = order['sql']) !== null && _a !== void 0 ? _a : this.wrap(order['column']) + ' ' + order['direction'];
        });
    }
    /**
    ...
    compileUpdate(query, values) {
        ...
        return (query.joins.length > 0
            ...
            : this.compileUpdateWithoutJoins(query, table, columns, where)).trim();
    }
    /**
    ...
    compileUpdateColumns(query, values) {
        return Helpers_1.collect(values).map(([key, value]) => {
    ...
    compileWheresToArray(query) {
        return Helpers_1.collect(query.wheres).map((where) => {
    ...
    concatenateWhereClauses(query, sql) {
        const conjunction = query instanceof JoinClause_1.JoinClause ? 'on' : 'where';
    ...
    prepareBindingsForUpdate(bindings, values) {
        ...
        const cleanBindings = lodash_1.reject(bindings, ['select', 'join']);
        return Object.values(
        // merge(bindings['join'], Object.values(values), Arr.flatten(cleanBindings))
        [...bindings['join'], ...Object.values(values), ...Collections_1.Arr.flatten(cleanBindings)]);
    }
    /**
    ...
    whereBetween(query, where) {
        ...
        const min = this.parameter(Helpers_1.reset(where.values));
        const max = this.parameter(Helpers_1.end(where.values));
    ...
    whereBetweenColumns(query, where) {
        const between = where['not'] ? 'not between' : 'between';
        const min = this.wrap(Helpers_1.reset(where.values));
        const max = this.wrap(Helpers_1.end(where.values));
        return this.wrap(where.column) + ' ' + between + ' ' + min + ' and ' + max;
    }
    /**
    ...
    whereIn(query, where) {
        var _a;
        if (((_a = where.values) === null || _a === void 0 ? void 0 : _a.length) > 0) {
    ...
    whereNested(query, where) {
        ...
        const offset = query instanceof JoinClause_1.JoinClause ? 3 : 6;
